...
- (void)initWithProperty
{
    ...
    self.titleSelectFont = kFont(15);
  //  self.titleFont = HX_FONT(14);
   // self.indicatorColor = self.titleSelectColor;
    self.indicatorExtension = 5.f;
    self.selectBoardColor = ClearColor;
    self.boardColor = ClearColor;
    self.backColor = ClearColor;
    self.selectBackColor = ClearColor;
}
//重新布局frame
- (void)layoutSubviews
{
    ...
    [UIView animateWithDuration:(animated?0.3:0) animations:^{
        switch (self->_indicatorType)
        {
            case FSIndicatorTypeDefault:
                ...
                self.indicatorView.center = CGPointMake(selectBtn.center.x,self.viewHeight-2);
                self.indicatorView.bounds = self->_button_Width>0 ?(CGRectMake(0, 0,self.button_Width,2)):(CGRectMake(0, 0,7, 4));
                if (self->_indicatorType == 2) {
...
- (void)setTitleNormalColor:(UIColor *)titleNormalColor
{
    ...
    for (UIButton *btn in self.itemBtnArr)
    {
        ...
        [btn setBackgroundImage:[UIImage imageWithColor:self.backColor] forState:UIControlStateNormal];
    }
}
...
- (void)setTitleSelectColor:(UIColor *)titleSelectColor
{
    ...
    for (UIButton *btn in self.itemBtnArr)
    {
        ...
        [btn setBackgroundImage:[UIImage imageWithColor:self.selectBackColor] forState:UIControlStateSelected];
    }
}
