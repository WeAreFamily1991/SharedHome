...
+ (NSInteger)getMonthHasDays:(NSDate *)findDate
{
    NSCalendar *calendar = [NSCalendar currentCalendar];
    NSRange range = [calendar rangeOfUnit:NSCalendarUnitDay inUnit:NSCalendarUnitMonth forDate:findDate];
    return range.length;
}
+ (NSInteger)getFirstWeekDayInMonth:(NSDate *)findDate
{
    NSCalendar *calendar = [NSCalendar currentCalendar];
    
    NSDateComponents *components = [calendar components:(NSCalendarUnitYear | NSCalendarUnitMonth | NSCalendarUnitDay) fromDate:findDate];
...
+ (NSInteger)getPreviousMonthDays {
    NSCalendar *calendar = [NSCalendar currentCalendar];
    ...
    NSInteger totalDays = [[NSCalendar currentCalendar] rangeOfUnit:NSCalendarUnitDay inUnit:NSCalendarUnitMonth forDate:previousDate].length;
    return totalDays;
}
...
+ (NSString *)getTimeStamp {
    ...
    **/
    ...
    NSString *timeSp0 = [NSString stringWithFormat:@"%.0f", interval];
    NZLog(@"lllll == %@",timeSp0);
  //  return timeSp;
...
//
    ...
    NZLog(@"PPPPPPP00 == %@",resultStr);
//    [parameters setValue:[XMToolClass md5:resultStr] forKey:@"API_SIGN"];
//    [parameters setValue:[XMToolClass getTimeStamp] forKey:@"API_TIMESTAMP"];
//    [parameters setValue:@"IOS" forKey:@"API_DEVICE"];
    
    NZLog(@"PPPPPPP == %@",parameters);
    return parameters;
}
...
+ (void)removeMessage:(NSString *)key
{
