...
@interface SHServiceHomePageVC ()<UICollectionViewDelegate,UICollectionViewDataSource>
...
@implementation SHServiceHomePageVC
...
-(void)setCollectionView
{
    ...
    ///纯代码
    [collectionView registerClass:[SHServiceHomePageCell class] forCellWithReuseIdentifier:cellID];
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        SHServiceHomePageCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:cellID forIndexPath:indexPath];
        cell.dict = @{};
       ...
       // cell.locationImgView.hidden = YES;
        MJWeakSelf;
        [cell setLocationBlock:^(BOOL isOpen) {
...
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath
{
    ...
    else if (kind == UICollectionElementKindSectionFooter)
    {
        ...
        if (self.listArray.count == 0 && indexPath.section == 1) {
            UILabel *emptyLabel = [UILabel labelWithText:@"暂无上传作品" font:kFont(14) textColor:UIColorFromRGB(0xb4b4b4) backGroundColor:ClearColor superView:footView];
            [emptyLabel mas_makeConstraints:^(MASConstraintMaker *make) {
                make.centerX.mas_equalTo(footView);
                make.top.mas_equalTo(WScale(140));
            }];
        }
        else
        {
...
{
    if (indexPath.section == 0) {
       float height = self.isOpen == YES?(WScale(258)):(WScale(155));
...
{
    if (section == 0) {
        return CGSizeMake(kWindowW,0.01);
    }
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout*)collectionViewLayout referenceSizeForFooterInSection:(NSInteger)section
{
    ...
    if (self.listArray.count == 0) {
        return CGSizeMake(kWindowW,WScale(200));
    }
    return CGSizeMake(kWindowW,0.01);
}
