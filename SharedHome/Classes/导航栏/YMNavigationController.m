...
- (void)viewDidLoad {
    ...
    [[UIBarButtonItem appearance] setTitleTextAttributes:@{NSForegroundColorAttributeName : WhiteColor} forState:UIControlStateNormal];

    self.delegate = self;
   // [self.navigationBar setShadowImage:[UIImage imageWithColor2:SeparatorCOLOR]];
...
- (void)pushViewController:(UIViewController *)viewController animated:(BOOL)animated
{
    ...
    if (self.viewControllers.count >= 1) {
        ...
        viewController.navigationItem.leftBarButtonItem = [UIBarButtonItem itemWithTarget:self action:@selector(back) image:[UIImage imageNamed:@"nav_back"] imageEdgeInsets:UIEdgeInsetsZero];
        
        // 调用自定义方法,使用上下文截图
        [self screenShot];
    }
    [viewController.navigationController.navigationBar setShadowImage:[UIImage imageWithColor2:SeparatorCOLOR]];
